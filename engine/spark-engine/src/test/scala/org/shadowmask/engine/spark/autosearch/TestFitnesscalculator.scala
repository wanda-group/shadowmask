...
class TestFitnesscalculator {
  ...
  @Test
  def testRepRddCalculator(): Unit ={
    val conf = new SparkConf().setAppName("shadowmask").setMaster("local")
    val sc = new SparkContext(conf)
    val rdd = sc.parallelize(1 to 100)
    val calculator = new  RddReplicatedSparkDrivedFitnessCalculator(8,4,4)
    for (i<-0 until 16){
      calculator.mapToNewDataSet(rdd.map(_.toString))
    }
    calculator.releaseRdd(0)
    calculator.releaseRdd(0)
